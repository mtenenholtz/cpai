...
function metricOfFile(f: FileEntry, metric: State["treeMetric"]) {
  return metric === "tokens" ? f.tokens : metric === "bytes" ? f.bytes : f.lines;
}

function metricOfDir(v: { tokens: number }, metric: State["treeMetric"]) {
...
async function loadSavedPrompts(cwd: string, dirHint?: string): Promise<SavedPrompt[]> {
