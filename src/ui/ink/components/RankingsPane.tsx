...
function padLeft(n: number | string, w: number): string {
  const s = String(n);
...
export function RankingsPane(props: {
  ...
  const { filesCol, dirsCol, rowCount } = useMemo(() => {
    ...
    const fileLines = topFiles.map((f) => ({
      name: padRight(f.relPath, nameWLocal),
      tok: padLeft(f.tokens, tokenWLocal),
    }));
    const dirLines = topDirs.map(([d, t]) => ({
      name: padRight(d || '.', nameWLocal),
      tok: padLeft(t, tokenWLocal),
    }));
    ...
    rows.push(
      <Box key={i}>
        <Box width={colW} marginRight={2}>
          <Box width={cursorW}>
            <Text inverse={selFiles}>{selFiles ? '› ' : '  '}</Text>
          </Box>
          <Box width={nameW}>
            <Text wrap="truncate-end" inverse={selFiles}>
              {f?.name ?? ''.padEnd(nameW, ' ')}
            </Text>
          </Box>
          <Box width={tokenW}>
        ...
        <Box width={colW}>
          <Box width={nameW}>
            <Text wrap="truncate-end" inverse={selDirs}>
              {d?.name ?? ''.padEnd(nameW, ' ')}
            </Text>
          </Box>
          <Box width={tokenW}>
  ...
  return (
    <Box flexDirection="column">{rows.length ? rows : <Text dimColor>(no rankings)</Text>}</Box>
  );
}
