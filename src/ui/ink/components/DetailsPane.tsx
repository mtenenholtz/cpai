...
export function DetailsPane(props: { state: State; treeMode: boolean; selectedIdx: number; height: number }) {
  const { state } = props;
  ...
  const body = useMemo(() => {
    if (props.treeMode) {
      ...
      const f = state.files[props.selectedIdx];
      if (!f) return <Text dimColor>(no files)</Text>;
      const excluded = state.manualExcluded.has(f.relPath) || state.autoDeselected.has(f.relPath);
      return (
        <Box flexDirection="column">
          <Text color="cyan">{f.relPath}</Text>
  ...
  }, [state, props.treeMode, props.selectedIdx]);
