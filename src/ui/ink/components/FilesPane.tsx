...
function padRightStr(s: string, w: number): string {
...
export function FilesPane(props: {
  ...
  const rowsData = useMemo(() => {
    ...
    return makeVisibleTree(root, s.treeExpanded, eligible).map((v) => {
      ...
      return {
        kind: v.kind,
        key: v.kind === 'dir' ? `d:${v.node.path}` : `f:${v.file.relPath}`,
        name,
        indent,
        mark,
        status,
      ...
      {slice.map((row, i) => {
        ...
        return (
          <Box key={(row as any).key} width={contentWidth}>
            ...
            <Box width={indentW}>
              <Text inverse={isSel}>{indentStr}</Text>
            </Box>
            <Box width={markW}>
              <Text
                inverse={isSel}
                color={
                  (row as any).status === 'mixed'
                    ? 'yellow'
                    : (row as any).status === 'included'
                      ? 'green'
                      : 'gray'
                }
              >
            ...
            <Box width={nameAvail}>
              <Text inverse={isSel}>{displayName}</Text>
            </Box>
            <Box width={tokenW}>
              <Text inverse={isSel}>{displayTok}</Text>
            </Box>
          </Box>
        );
      })}
